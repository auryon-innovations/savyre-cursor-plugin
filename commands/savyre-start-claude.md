...
1. Run `savyre-start` / `savyre-guard.mjs start` from the project workspace, passing extra words after start as arguments. Follow JSON `stageId`. Extra text is the Stage 01 product leftover; it is ignored unless the panel is on Stage 01. If JSON already captured a task, do not ask what to build.
