...
**Reply to the user with only JSON `userMessage`.** That line is the WP4 review (short). Do not paste JSON. Do not quote `message`. Follow `message` yourself. Speak `composer.details` or `composer.status` only if they ask. Do not mention `final.md`, `ai-output.md`, `input.md`, `developer-review.md`, artifact, or ACCEPTED. Slash commands are OK.
